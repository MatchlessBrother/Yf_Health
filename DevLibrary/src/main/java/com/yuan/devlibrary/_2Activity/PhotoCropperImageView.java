...
public class PhotoCropperImageView extends ImageView {
    private static final String TAG = PhotoCropperImageView.class.getSimpleName();
    ...
    // Constructor /////////////////////////////////////////////////////////////////////////////////
    public PhotoCropperImageView(Context context) {
        this(context, null);
    }

    public PhotoCropperImageView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PhotoCropperImageView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void handleStyleable(Context context, AttributeSet attrs, int defStyle, float mDensity) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.PhotoCropperImageView, defStyle, 0);
        Drawable drawable;
        mCropMode = CropMode.RATIO_1_1;
        try {
            drawable = ta.getDrawable(R.styleable.PhotoCropperImageView_imgSrc);
            if (drawable != null) setImageDrawable(drawable);
            for (CropMode mode : CropMode.values()) {
                if (ta.getInt(R.styleable.PhotoCropperImageView_cropMode, 3) == mode.getId()) {
                    mCropMode = mode;
                    break;
                }
            }
            mBackgroundColor = ta.getColor(R.styleable.PhotoCropperImageView_backgroundColor, TRANSPARENT);
            super.setBackgroundColor(mBackgroundColor);
            mOverlayColor = ta.getColor(R.styleable.PhotoCropperImageView_overlayColor, TRANSLUCENT_BLACK);
            mFrameColor = ta.getColor(R.styleable.PhotoCropperImageView_frameColor, WHITE);
            mHandleColor = ta.getColor(R.styleable.PhotoCropperImageView_handleColor, WHITE);
            mGuideColor = ta.getColor(R.styleable.PhotoCropperImageView_guideColor, TRANSLUCENT_WHITE);
            for (ShowMode mode : ShowMode.values()) {
                if (ta.getInt(R.styleable.PhotoCropperImageView_guideShowMode, 1) == mode.getId()) {
                    mGuideShowMode = mode;
                    break;
                }
            }
            ...
            for (ShowMode mode : ShowMode.values()) {
                if (ta.getInt(R.styleable.PhotoCropperImageView_handleShowMode, 1) == mode.getId()) {
            ...
            mHandleSize = ta.getDimensionPixelSize(R.styleable.PhotoCropperImageView_handleSize,
                    (int) (HANDLE_SIZE_IN_DP * mDensity));
            mTouchPadding = ta.getDimensionPixelSize(R.styleable.PhotoCropperImageView_touchPadding, 0);
            mMinFrameSize = ta.getDimensionPixelSize(R.styleable.PhotoCropperImageView_minFrameSize,
                    (int) (MIN_FRAME_SIZE_IN_DP * mDensity));
            mFrameStrokeWeight = ta.getDimensionPixelSize(
                    R.styleable.PhotoCropperImageView_frameStrokeWeight,
                    (int) (FRAME_STROKE_WEIGHT_IN_DP * mDensity));
            mGuideStrokeWeight = ta.getDimensionPixelSize(
                    R.styleable.PhotoCropperImageView_guideStrokeWeight,
                    (int) (GUIDE_STROKE_WEIGHT_IN_DP * mDensity));
            mIsCropEnabled = ta.getBoolean(R.styleable.PhotoCropperImageView_cropEnabled, true);
            mInitialFrameScale = constrain(ta.getFloat(R.styleable.PhotoCropperImageView_initialFrameScale,
