...
/********此控件因为本身的设计问题导致无法添加展开/关闭父标题的功能,因为父类标题的生成是根据子类数据生成的,********/
/***一个父类标题的生成至少要包含一个子类数据.而子类数据对应的子类视图是无法完全隐藏的,从而导致无法正常关闭父标题**/
/********最后导致无法为此控件添加展开/关闭父标题的功能.但是如果一定要实现父标题的展开/关闭功能,个人建议采用*******/
/********StickyHeadersViewExpandableListView控件去实现功能,虽然显示速度会卡一点,但还可以凑合性的满足需求!*********/
/******强烈注意:使用系统自带的gridView控件时,我们需要控制每一个item必须一样高,否则滚动内容时会出现白屏的情况******/
public class StickyHeadersViewGridView extends GridView implements OnScrollListener, OnItemClickListener, OnItemSelectedListener, OnItemLongClickListener
{
    ...
    static final String TAG = StickyHeadersViewGridView.class.getSimpleName();
    ...
    private boolean mStickyHeadersTransparentWhenMoving = false;
    ...
    protected StickyHVGridViewBaseAdapter mAdapter;
    ...
    public StickyHeadersViewGridView(Context context) {
        this(context, null);
    }

    public StickyHeadersViewGridView(Context context, AttributeSet attrs) {
        this(context, attrs, android.R.attr.gridViewStyle);
    }

    public StickyHeadersViewGridView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        StickyHVGridViewListAdapter baseAdapter;
        if (adapter instanceof StickyHVGridViewListAdapter) {
            baseAdapter = (StickyHVGridViewListAdapter)adapter;
        } else if (adapter instanceof StickyHVGridViewSAListAdapter) {
            // Wrap up simple adapter to auto-generate the data we need.
            baseAdapter = new StickyHVGridViewSCALListArrayAdapter(
                    (StickyHVGridViewSAListAdapter)adapter);
        } else {
            // Wrap up a list adapter so it is an adapter with zero headers.
            baseAdapter = new StickyHVGridViewListBaseAdapter(adapter);
        }

        this.mAdapter = new StickyHVGridViewBaseAdapter(getContext(), this, baseAdapter);
    ...
    public boolean getStickyHeadersTransparentWhenMoving() {
        return mStickyHeadersTransparentWhenMoving;
    }

    public void setStickyHeadersTransparentWhenMoving(boolean mStickyHeadersTransparentWhenMoving) {
        this.mStickyHeadersTransparentWhenMoving = mStickyHeadersTransparentWhenMoving;
    }
    ...
    private int findMotionHeader(float y) {
        ...
        for (int i = getFirstVisiblePosition(); i <= getLastVisiblePosition();) {
            ...
            if (id == StickyHVGridViewBaseAdapter.ID_HEADER) {
    ...
    private void scrollChanged(int firstVisibleItem) {
        ...
        if (childCount != 0) {
            ...
            for (int i = 0; i < childCount; i += mNumMeasuredColumns) {
                ...
                if (mAdapter.getItemId(getPositionForView(child)) == StickyHVGridViewBaseAdapter.ID_HEADER
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        for (int i = 0; i < headerPositions.size(); i++) {
            ...
            boolean headerIsStickied = ((StickyHVGridViewBaseAdapter.HeaderFillerView)frame).getHeaderId() == mCurrentHeaderId
        ...
        {
            if(mStickyHeadersTransparentWhenMoving)
