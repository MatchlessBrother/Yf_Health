 ...
 */
public class StickyHeadersViewBListView extends FrameLayout
{
    /**
    ...
    public interface OnStickyHeaderOffsetChangedListener {
         ...
         */
        void onStickyHeaderOffsetChanged(StickyHeadersViewBListView l, View header, int offset);
    }
    ...
    public interface OnStickyHeaderChangedListener {
         ...
         */
        void onStickyHeaderChanged(StickyHeadersViewBListView l, View header,
                                   int itemPosition, long headerId);

    }

    /* --- Children --- */
    private ItemBListView mList;
    ...
    private StickyHVAListViewAdapter mAdapter;
    ...
    public StickyHeadersViewBListView(Context context) {
        this(context, null);
    }

    public StickyHeadersViewBListView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.StickyHeadersViewStyle);
    }

    @TargetApi(Build.VERSION_CODES.HONEYCOMB)
    public StickyHeadersViewBListView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        mTouchSlop = ViewConfiguration.get(getContext()).getScaledTouchSlop();

        // Initialize the wrapped list
        mList = new ItemBListView(context);
        ...
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.StickyHeadersView, defStyle, 0);

            try {
                // -- View attributes --
                int padding = a.getDimensionPixelSize(R.styleable.StickyHeadersView_android_padding, 0);
                mPaddingLeft = a.getDimensionPixelSize(R.styleable.StickyHeadersView_android_paddingLeft, padding);
                mPaddingTop = a.getDimensionPixelSize(R.styleable.StickyHeadersView_android_paddingTop, padding);
                mPaddingRight = a.getDimensionPixelSize(R.styleable.StickyHeadersView_android_paddingRight, padding);
                mPaddingBottom = a.getDimensionPixelSize(R.styleable.StickyHeadersView_android_paddingBottom, padding);

                setPadding(mPaddingLeft, mPaddingTop, mPaddingRight, mPaddingBottom);

                // Set clip to padding on the list and reset value to default on
                // wrapper
                mClippingToPadding = a.getBoolean(R.styleable.StickyHeadersView_android_clipToPadding, true);
                super.setClipToPadding(true);
                mList.setClipToPadding(mClippingToPadding);

                // scrollbars
                final int scrollBars = a.getInt(R.styleable.StickyHeadersView_android_scrollbars, 0x00000200);
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
                    mList.setOverScrollMode(a.getInt(R.styleable.StickyHeadersView_android_overScrollMode, 0));
                }

                // -- ListView attributes --
                mList.setFadingEdgeLength(a.getDimensionPixelSize(R.styleable.StickyHeadersView_android_fadingEdgeLength,
                        mList.getVerticalFadingEdgeLength()));
                final int fadingEdge = a.getInt(R.styleable.StickyHeadersView_android_requiresFadingEdge, 0);
                ...
                mList.setCacheColorHint(a
                        .getColor(R.styleable.StickyHeadersView_android_cacheColorHint, mList.getCacheColorHint()));
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                    mList.setChoiceMode(a.getInt(R.styleable.StickyHeadersView_android_choiceMode,
                            mList.getChoiceMode()));
                }
                mList.setDrawSelectorOnTop(a.getBoolean(R.styleable.StickyHeadersView_android_drawSelectorOnTop, false));
                mList.setFastScrollEnabled(a.getBoolean(R.styleable.StickyHeadersView_android_fastScrollEnabled,
                        mList.isFastScrollEnabled()));
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                    mList.setFastScrollAlwaysVisible(a.getBoolean(
                            R.styleable.StickyHeadersView_android_fastScrollAlwaysVisible,
                            mList.isFastScrollAlwaysVisible()));
                }
                mList.setScrollBarStyle(a.getInt(R.styleable.StickyHeadersView_android_scrollbarStyle,View.SCROLLBARS_INSIDE_OVERLAY));

                if (a.hasValue(R.styleable.StickyHeadersView_android_listSelector)) {
                    mList.setSelector(a.getDrawable(R.styleable.StickyHeadersView_android_listSelector));
                }

                mList.setScrollingCacheEnabled(a.getBoolean(R.styleable.StickyHeadersView_android_scrollingCache,
                        mList.isScrollingCacheEnabled()));

                if (a.hasValue(R.styleable.StickyHeadersView_android_divider)) {
                    mDivider = a.getDrawable(R.styleable.StickyHeadersView_android_divider);
                }
                
                mList.setStackFromBottom(a.getBoolean(R.styleable.StickyHeadersView_android_stackFromBottom, false));

                mDividerHeight = a.getDimensionPixelSize(R.styleable.StickyHeadersView_android_dividerHeight,
                        mDividerHeight);

                mList.setTranscriptMode(a.getInt(R.styleable.StickyHeadersView_android_transcriptMode,
                        ListView.TRANSCRIPT_MODE_DISABLED));

                // -- StickyListHeaders attributes --
                mAreHeadersSticky = a.getBoolean(R.styleable.StickyHeadersView_hasStickyHeaders, true);
                mIsDrawingListUnderStickyHeader = a.getBoolean(
                        R.styleable.StickyHeadersView_isDrawingListUnderStickyHeader,
                        true);
            } finally {
                a.recycle();
            }
    ...
    private void updateHeader(int headerPosition) {
        ...
        for (int i = 0; i < mList.getChildCount(); i++) {
            ...
            final boolean doesChildHaveHeader = child instanceof ItemAView && ((ItemAView) child).hasHeader();
            ...
            mHeader.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    mOnHeaderClickListener.onHeaderClick(
                            StickyHeadersViewBListView.this, mHeader,
    ...
    private void updateHeaderVisibilities() {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (!(child instanceof ItemAView)) {
                continue;
            }

            // ensure wrapper view child has a header
            ItemAView wrapperViewChild = (ItemAView) child;
    ...
    private class WrapperViewListLifeCycleListener implements ItemBListView.LifeCycleListener {
    ...
    private class AdapterWrapperHeaderClickHandler implements StickyHVAListViewAdapter.OnStickyHeaderViewClickListener
    {
        @Override
        public void onStickyHeaderViewClick(View header, int itemPosition, long headerId)
        {
            mOnHeaderClickListener.onHeaderClick(StickyHeadersViewBListView.this, header, itemPosition, headerId, false);
        }
    ...
    public void setAdapter(StickyHVAdapter adapter) {
        if (adapter == null) {
            if (mAdapter instanceof StickyHVBSIListViewAdapter) {
                ((StickyHVBSIListViewAdapter) mAdapter).mSectionIndexerDelegate = null;
            }
            if (mAdapter != null) {
                mAdapter.mStickyHeadersAdapter = null;
            }
        ...
        if (adapter instanceof SectionIndexer) {
            mAdapter = new StickyHVBSIListViewAdapter(getContext(), adapter);
        } else {
            mAdapter = new StickyHVAListViewAdapter(getContext(), adapter);
        }
    ...
    public StickyHVAdapter getAdapter() {
        return mAdapter == null ? null : mAdapter.mStickyHeadersAdapter;
    }
        ...
        if (l != null) {
            mList.setOnTouchListener(new OnTouchListener() {
                ...
                public boolean onTouch(View v, MotionEvent event) {
                    return l.onTouch(StickyHeadersViewBListView.this, event);
                }
            });
        } else {
